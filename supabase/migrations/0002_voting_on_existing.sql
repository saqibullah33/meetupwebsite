-- Compatible with the existing remote project (profiles already exists).
-- Adds meetup voting tables, extends profiles with `name`, and keeps
-- handle_new_user working for both `name` and `full_name` metadata.
...
alter table public.profiles
  alter column name set default 'Participant';
...
drop policy if exists "Users can upload their project images" on storage.objects;
